...
class PetitionConfirmStep extends ConfirmStep {
    ...
    if (isset($this->wizard->node->nid)) {
      if(isset($form_state['clicked_button']['#name'])) {
        ...
        case 'finish':
          $node->status = 1;
          node_save($node);
          drupal_set_message(t('Petition published successfully.'), 'status');
          $form_state['redirect'] = 'node/' . $node->nid;
          break;
        case 'draft':
          $node->status = 0;
          node_save($node);
          drupal_set_message(t('Petition saved as draft.'), 'status');
          $form_state['redirect'] = 'node/' . $node->nid;
          break;
        case 'schedule':
